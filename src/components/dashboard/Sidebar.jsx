...
const Sidebar = ({ activeSection, setActiveSection, isCollapsed, setIsCollapsed }) => {
  ...
  const menuItems = [
    ...
    { id: 'courses', label: 'My Courses', icon: BookOpen, color: 'text-red-600' },
  ...
  const quickStats = [
    { label: 'Courses', value: '3', icon: BookOpen, color: 'bg-red-100 text-red-600' },
  ...
  return (
    <div className={`${isCollapsed ? 'w-20' : 'w-72'} bg-white/90 backdrop-blur-sm border-r border-red-200 h-screen sticky top-0 transition-all duration-300 flex flex-col shadow-lg`}>
      {/* Header */}
            ...
            <button
              ...
              className={`w-full flex items-center ${isCollapsed ? 'justify-center px-3' : 'px-4'} py-3 rounded-xl transition-all duration-200 group relative ${
                activeSection === item.id
                  ? 'bg-gradient-to-r from-red-100 to-blue-100 text-red-700 shadow-sm'
